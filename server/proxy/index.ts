...
export async function getEps_articles(params) {
    let page = 1;
    const pageSize = parseInt(params.pageSize);
    page = parseInt(params.pageIndex) || 1;
    page = page > 0 ? page : 1;
...
export async function getComments(params) {
    ...
    const data = await Promise.all([Comment.find(query, {}, options).exec(), Comment.countDocuments(query).exec()]);
...
export async function getGuestbook(params) {
    ...
    options.limit = pageSize;
    options.sort = '-createTime';
    const data = await Promise.all([Guestbook.find({}, {}, options).exec(), Guestbook.countDocuments({}).exec()]);
    const comments = data[0];
    const count = data[1];
    const pageCount = Math.ceil(count / pageSize);
    return {
