...
export async function getPosts(params: any) {
    const matchObj: any = {};
    const { category, alias, title, content, label, createTime, modifyTime, publishTime, isLink, isDraft, hasComments, isDeleted } = params;
    if (category) {
    ...
    if (Array.isArray(createTime) && createTime.length === 2 && createTime[0] && createTime[1]) {
        const start = new Date(createTime[0]);
        const end = new Date(createTime[1]);
        matchObj.createTime = { $gte: start, $lt: end };
    }
    if (Array.isArray(modifyTime) && modifyTime.length === 2 && modifyTime[0] && modifyTime[1]) {
        const start = new Date(modifyTime[0]);
        const end = new Date(modifyTime[1]);
        matchObj.modifyTime = { $gte: start, $lt: end };
    }
    if (Array.isArray(publishTime) && publishTime.length === 2 && publishTime[0] && publishTime[1]) {
...
export async function deleteArticle(uids: Array<string> | string) {
    ...
    const result = await Post.updateMany({ _id: { $in: uids } }, { isActive: false, publishTime: null }).exec();
    return {
        result
    };
}
...
export async function delete2Article(uid) {
    const result = await Promise.all([Post.findByIdAndDelete(uid).exec(), Comment.deleteMany({ post: uid }).exec()]);
    return {
        result
    };
}
...
export async function getGuestbook(params) {
    ...
    if (Array.isArray(params.createTime) && params.createTime.length === 2 && params.createTime[0] && params.createTime[1]) {
        const start = new Date(params.createTime[0]);
        const end = new Date(params.createTime[1]);
        query.createTime = { $gte: start, $lt: end };
    }
    const data = await Promise.all([Guestbook.find(query, {}, options).exec(), Guestbook.countDocuments(query).exec()]);
    return {
        comments: data[0],
        count: data[1]
    };
