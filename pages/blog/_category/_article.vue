                ...
                <div v-show="menuShow" class="side-block-container">
                    ...
                    <a-anchor :affix="false" :show-ink-in-fixed="true" :offset-top="75" :bounds="10">
                        <a-anchor-link v-for="(item1, index1) in menus" :key="index1" :href="item1.href" :title="item1.title">
                            <a-anchor-link v-for="(item2, index2) in item1.subs" :key="index2" :href="item2.href" :title="item2.title" />
                        </a-anchor-link>
                    </a-anchor>
                </div>
                <pop-articles />
    ...
    methods: {
        ...
        scrollByHash() {
            ...
            if (hash.indexOf(prefix) === 0) {
                history.replaceState(null, '', location.href.replace(/#.*/, '') + '#' + hash.replace(prefix, ''));
            } else {
                const anchor = document.querySelector(`#${prefix}${hash}`);
                if (anchor) {
                    window.scrollTo(window.scrollX, anchor.getBoundingClientRect().top + window.scrollY - 75);
                }
            }
        }
    },
