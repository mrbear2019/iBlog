        ...
        <div v-if="profile" class="profile-wrap">
            ...
            <div class="main-wrap">
                ...
                <div v-if="profile.github || profile.email" class="footer">
                    <a v-if="profile.github" :href="profile.github" title="GitHub" target="_blank"
                        ><font-awesome-icon :icon="['fab', 'github']" /> {{ profile.github }}</a
                    >
    ...
    async asyncData({ $axios, error }: Context) {
        ...
        if (code === 1) {
            ...
            profile.introduction = md
                .render(profile.introduction)
                .replace(/:([\w-]+?):/g, '<i class="icon-emoji" style="background-image: url(/images/emojis/$1.png);"></i>')
            return {
                profile
            }
        } else {
