<template>
    <div>
        ...
        <div class="page-body">
                ...
                <a-form :form="form" :self-update="true">
                    <a-row>
                        <a-col :sm="24" :md="11" :xxl="5">
                            <a-form-item label="分" :colon="false">
                                <a-select v-decorator="['category', categoryOpts]" placeholder="不限" allow-clear>
                                    <a-select-option v-for="(item, index) in categories" :key="index" :value="item._id">
                                        {{ item.cateName }}
                                    </a-select-option>
                                </a-select>
                            </a-form-item>
                        ...
                        <a-col :sm="24" :md="{ span: 11, offset: 2 }" :xxl="{ span: 5, offset: 1 }">
                            <a-form-item label="Alias" :colon="false">
                                <a-input v-decorator="['alias']" placeholder="文章Alias" allow-clear />
                            </a-form-item>
                        </a-col>
                        <a-col :sm="24" :md="11" :xxl="{ span: 5, offset: 1 }">
                            <a-form-item label="标题" :colon="false">
                                <a-input v-decorator="['title']" placeholder="标题关键字" allow-clear />
                            </a-form-item>
                        </a-col>
                        <a-col :sm="24" :md="{ span: 11, offset: 2 }" :xxl="{ span: 5, offset: 1 }">
                            <a-form-item label="全文" :colon="false">
                                <a-input v-decorator="['content']" placeholder="全文关键字" allow-clear />
                            </a-form-item>
                        </a-col>
                        <a-col :sm="24" :md="11" :xxl="5">
                            <a-form-item label="标签" :colon="false">
                                <a-input v-decorator="['label']" placeholder="标签关键字" allow-clear />
                            </a-form-item>
                        </a-col>
                        <a-col :sm="24" :md="{ span: 11, offset: 2 }" :xxl="{ span: 5, offset: 1 }">
                        ...
                        <a-col :sm="24" :md="11" :xxl="5">
                            <a-form-item label="是否外链" :colon="false">
                                <a-select v-decorator="['isLink']" placeholder="不限" allow-clear>
                        ...
                        <a-col :sm="24" :md="{ span: 11, offset: 2 }" :xxl="{ span: 5, offset: 1 }">
                            <a-form-item label="是否草稿" :colon="false">
                                <a-select v-decorator="['isDraft', isDraftOpts]" placeholder="不限" allow-clear>
                                    <a-select-option value="1">
                                        是
                                    </a-select-option>
                                    <a-select-option value="-1">
                        ...
                        <a-col :sm="24" :md="11" :xxl="{ span: 5, offset: 1 }">
                            <a-form-item label="是否有评论" :colon="false">
                                <a-select v-decorator="['hasComments']" placeholder="不限" allow-clear>
                                    <a-select-option value="1">
                                        是
                                    </a-select-option>
                                    <a-select-option value="-1">
                        ...
                        <a-col :sm="24" :md="{ span: 11, offset: 2 }" :xxl="{ span: 5, offset: 1 }">
                            <a-form-item label="是否已删除" :colon="false">
                                <a-select v-decorator="['isDeleted', isDeletedOpts]" placeholder="不限" allow-clear>
                                    <a-select-option value="1">
                                        是
                                    </a-select-option>
                                    <a-select-option value="-1">
                    ...
                    <a-row type="flex" justify="center">
                        <a-col>
                            <a-button type="primary" @click="search"> <font-awesome-icon :icon="['fas', 'search']" style="margin-right: 4px;" />搜索 </a-button>
            ...
            <div class="data-wrap">
                ...
                >
                    <template slot="category" slot-scope="text, row">
                        <a class="link-category" :href="`/blog/${row.categories[0].alias}`" target="_blank" :title="row.categories[0].cateName">{{
                            row.categories[0].cateName
                        }}</a>
                    </template>
                    ...
                    <template slot="tags" slot-scope="text, row">
                        <a-tag v-if="!row.isActive" color="volcano" title="已删除，所有人不可见，可恢复">
                            已删除
                        </a-tag>
                        <a-tag v-else-if="!row.isDraft" color="green" title="已发布，所有人可见">
                            已发布
                        </a-tag>
                    ...
                    <template slot="publishTime" slot-scope="text, row">
                        <span v-if="row.publishTime"
                            >{{ row.publishTime | toDate }}
                            <font-awesome-icon class="icon-edit-time" :icon="['fas', 'pencil-alt']" @click="editPublishTime(row)"
                        /></span>
                        <span v-else>-</span>
                    </template>
                    <template slot="commentsCount" slot-scope="text, row">
                        <nuxt-link v-if="row.commentsCount > 0" :to="`/admin/comment-manage?alias=${row.alias}`" style="font-weight: 500;">
                            {{ row.commentsCount }}
                        </nuxt-link>
                        <span v-else>0</span>
                    </template>
                    <template slot="action" slot-scope="text, row">
                        <div class="action-td">
                            <template v-if="row.isActive">
                                <nuxt-link class="ant-btn" :to="`/admin/article-edit?uid=${row._id}`" title="编辑">
            ...
            <a-modal v-model="editingTimeObj.modal" title="自定义发布时间" :confirm-loading="editingTimeObj.loading" :closable="false" @ok="savePublishTime">
    ...
    methods: {
        ...
        async getList() {
            ...
            if (createTimeMomentArr && createTimeMomentArr.length === 2) {
                createTime = [createTimeMomentArr[0].startOf('day').toString(), createTimeMomentArr[1].endOf('day').toString()];
            }
            ...
            if (modifyTimeMomentArr && modifyTimeMomentArr.length === 2) {
                modifyTime = [modifyTimeMomentArr[0].startOf('day').toString(), modifyTimeMomentArr[1].endOf('day').toString()];
            }
            const publishTimeMomentArr = values.publishTime;
            let publishTime: string[] | undefined;
            if (publishTimeMomentArr && publishTimeMomentArr.length === 2) {
                publishTime = [publishTimeMomentArr[0].startOf('day').toString(), publishTimeMomentArr[1].endOf('day').toString()];
            }
