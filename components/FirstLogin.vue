    ...
    <div class="auth-panel">
        ...
        <a-form label-position="top" :form="form" class="form">
            <a-form-item label="密码" :colon="false">
                <a-input-password ref="pwd1" v-decorator="['pwd1', pwd1Opts]" size="large" placeholder="请输入密码" />
            </a-form-item>
            <a-form-item label="确认密码" :colon="false">
                <a-input-password v-decorator="['pwd2', pwd2Opts]" size="large" placeholder="请再次输入密码" @keyup.enter="save" />
            </a-form-item>
        </a-form>
        ...
        save(this: any) {
            this.form.validateFields(async (error, values) => {
                if (!error) {
                    ...
                    const { code, message }: IResp = await this.$axios.$put('/api/auth/account', {
